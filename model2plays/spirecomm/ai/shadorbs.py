...
#a neural net with output values for all cards
class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.fc1 = nn.Linear((len(cardNames)+1) * 10 + 5, 2000)
...
#a neural net with output values only for playable cards
class Net2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x * canplay
        x = self.softmax(x)
        x = torch.concat((x, torch.zeros((x.size()[0], 1)).to(self.device)), 1)
        return x[0]
...
class SimpleAgent:
    ...
    def __init__(self, chosen_class=PlayerClass.THE_SILENT):
        ...
        #load neural net
        self.net = Net2(self.device)
        if(self.gpu):
            self.net.to(self.device)
        self.net.load_state_dict(torch.load('shadorbs/model'))
    ...
    def card2tensor(self, card, upgraded):
        if not card in cardNames:
            return torch.zeros((len(cardNames)+1))
        card = cardIDs[card]
    ...
    def ask_shadorbs(self):
        ...
        #set score of unplayable cards to zero
        plays = np.zeros_like(pred)
        plays[-1] = pred[-1] # set upgrade
        ...
        #only keep the type of card that we want to play
        id_to_play = np.argmax(plays[:-1])
        ...
        if upgr and nonupgr:
            upgr_pref = 1 if plays[-1] > 0.5 else 0
    ...
    def get_play_card_action(self):
        ...
        cont, card_to_play = self.ask_shadorbs()
        if not cont:
            return EndTurnAction()
